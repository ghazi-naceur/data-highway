...
object ElasticSink extends ElasticUtils {
  ...
  def sendToElasticsearch(
    ...
    Either.catchNonFatal {
      ...
      FilesUtils.movePathContent(in, basePath, JSON)
    }
  }
  ...
  private def indexWithIndexQuery(in: String, out: String, basePath: String): Any = {
    if (new File(in).isFile) {
      ...
      FilesUtils.movePathContent(in, basePath, JSON, s"processed/$suffix")
    } else {
      FilesUtils
        ...
        .foreach(file => {
          ...
          FilesUtils.movePathContent(file.getAbsolutePath, basePath, JSON, s"processed/$suffix")
        })
    }
  }
