...
class XlsxCsvConverterSpec
  ...
  "XlsxCsvConverter.convertXlsxFileToCsvFiles" should "convert xlsx file to multiple csv files" in {
    ...
    val inputStream =
      new FileInputStream(folder + "input/mock-xlsx-data-1.xlsx")
    XlsxCsvConverter.convertXlsxFileToCsvFiles("mock-xlsx-data-1",
                                               inputStream,
                                               folder + "output/")
  ...
  "XlsxCsvConverter.apply" should "convert xlsx files to multiple csv files" in {
    ...
    XlsxCsvConverter.apply(folder + "input/",
                           folder + "output/",
                           Seq(XLSX_EXTENSION, XLSX_EXTENSION))
    val list1 = List(
      "data1.csv",
      "data2.csv",
      "data3.csv",
  ...
  "XlsxCsvConverter.createPathRecursively" should "create a directory and its subdirectories" in {
    val str = XlsxCsvConverter.createPathRecursively(
